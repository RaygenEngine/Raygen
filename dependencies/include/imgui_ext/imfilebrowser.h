...
public:
	...
	void ClearSelected(std::string filename = "");
...
inline void ImGui::FileBrowser::Open(std::string filename)
{
	ClearSelected(filename);
				...
				if (selected) {
					...
					if ((rsc.isDir && (flags_ & ImGuiFileBrowserFlags_SelectDirectory))
						...
						if (!(flags_ & ImGuiFileBrowserFlags_SelectDirectory))
							strcpy_s(inputNameBuf_->data(), INPUT_NAME_BUF_SIZE, selectedFilename_.c_str());
					}
				}
			}
...
inline void ImGui::FileBrowser::ClearSelected(std::string filename)
{
