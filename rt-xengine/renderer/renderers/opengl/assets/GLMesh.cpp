...
{
	GLMesh::GLMesh(GLRendererBase* renderer)
		: GLAsset(renderer), m_vbo(0), m_ebo(0)
	{
	}
	...
	bool GLMesh::Load(Assets::XMesh* data, GLenum usage)
	{
		...
		for (; matOffset != materialOffsets.end() && material != materialsInOffsetOrder.end(); ++matOffset, ++material)
		{
			...
			ggg.material = GetRenderer()->RequestGLMaterial(*material);
