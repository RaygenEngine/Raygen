#ifndef MULTIKEYASSETCACHEHASHING_H
#define MULTIKEYASSETCACHEHASHING_H
...
// create multi key hashing maps: std::unordered_map<Key<Types...>, V> (e.g. std::unordered_map<int, bool>, optix::Geometry>
// !each type must have an std::hash<Type>()(item) and an operator== overload otherwise there will be a compiler error!
...
namespace Assets
{
	...
	std::shared_ptr<AssetType> LoadAssetAtMultiKeyCache(MultiKeyAssetCache<MapAssetType, Args...>& cache, ContextType* context, Args ...args)
	{
		...
		auto asset = std::shared_ptr<AssetType>(new AssetType(context), [key, &cache](AssetType* assetPtr)
		{
			// custom deleter to remove from filesystem asset cache
...
#endif // MULTIKEYASSETCACHEHASHING_H
