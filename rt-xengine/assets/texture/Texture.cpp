...
Texture::Texture(AssetManager* assetManager, const std::string& path)
...
std::unique_ptr<Texture> Texture::CreateDefaultTexture(AssetManager* assetManager, void* texelValue, uint32 width,
	uint32 height, uint32 components, DynamicRange dr, const std::string& defaultName)
{
	...
	std::unique_ptr<Texture> texture = std::make_unique<Texture>(assetManager, defaultName);
