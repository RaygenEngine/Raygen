...
inline TriangleModelInstancedGeometryNode* LoadInstancingMatrixMetaNode(Node* parent, const tinyxml2::XMLElement* xmlElement)
{
	...
	auto interval = 5.f;
	ReadFloatsAttribute<float>(xmlElement, "interval", interval);

	glm::vec3 translation{ 0.f, 0.f, 0.f };
	ReadFloatsAttribute(xmlElement, "translation", translation);
	glm::vec3 eulerPYR{ 0.f, 0.f, 0.f };
	ReadFloatsAttribute(xmlElement, "euler_pyr", eulerPYR);
	auto orientation = glm::quat(glm::radians(eulerPYR));
	glm::vec3 scale{ 1.f, 1.f, 1.f };
	ReadFloatsAttribute(xmlElement, "scale", scale);

	std::string name = {};
	ReadStringAttribute(xmlElement, "name", name);
	std::string file = {};
	ReadStringAttribute(xmlElement, "file", file);
