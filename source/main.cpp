...
// 2) create the engine and engine->InitDirectories(..) given appropriate asset directory name (use argv[0] for application dir)
// 3) load a world using the engine->CreateWorldFromFile(..) (path to scene xml file)
// 4) use the renderer registration system found inside the engine container to register one or more renderers (and obtain their registration indices)
// 5) start the renderer by performing the following calls in the order below (you can use && short circuiting as those are all bool functions)
//    a) engine->SwitchRenderer(registrationIndex); (call requires registered renderers) 
//    b) renderer->InitRendering(..); (call may require window target - currently general Renderer abstraction has a virtual Windows-based function) 
//    c) renderer->InitScene(..); (call requires world)
// 6) (suggestion) before the mainloop it is wise to unload all the disk assets (main memory) (however if you want to switch fast between renderers, don't
// 7) main loop should be generally constructed as follows:
