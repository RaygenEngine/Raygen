...
namespace detail
{
	...
	std::function<void(void*, enum_under_t)> CreateGenericEnumSetter()
	{
		...
		return [](void* enumPtr, enum_under_t value) {
			auto optV = magic_enum::enum_cast<T>(
				static_cast<magic_enum::underlying_type_t<T>>(value));
...
public:
	...
	[[nodiscard]]
	const std::map<under_t, std::string_view>& GetValues() const
	{
		return valueToStr;
	}
	...
	template<typename T>
	[[nodiscard]]
	MetaEnumInst TieEnum(T& obj) const
	{
