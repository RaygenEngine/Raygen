...
namespace OpenGL
{
	...
	bool GLDeferredRenderer::InitScene(int32 width, int32 height)
	{
		...
		auto shaderAsset = AssetManager::GetOrCreate<ShaderPod>("gBuffer.shader.json");
		m_gBuffer.shader = GetGLAssetManager()->GetOrMakeFromUri<GLShader>(Engine::GetAssetManager()->GetPodPath(shaderAsset));
		...
		shaderAsset = AssetManager::GetOrCreate<ShaderPod>("gBufferAlphaMask.shader.json");
		m_gBuffer.shaderAlphaMask = GetGLAssetManager()->GetOrMakeFromUri<GLShader>(Engine::GetAssetManager()->GetPodPath(shaderAsset));
		...
		shaderAsset = AssetManager::GetOrCreate<ShaderPod>("deferred_directional_light.shader.json");
		m_deferredDirectionalLightShader = GetGLAssetManager()->GetOrMakeFromUri<GLShader>(Engine::GetAssetManager()->GetPodPath(shaderAsset));
		...
		shaderAsset = AssetManager::GetOrCreate<ShaderPod>("deferred_spot_light.shader.json");
		m_deferredSpotLightShader = GetGLAssetManager()->GetOrMakeFromUri<GLShader>(Engine::GetAssetManager()->GetPodPath(shaderAsset));
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
		glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, m_gBuffer.metallicRoughnessOcclusionOcclusionStengthAttachment, 0);
	...
	void GLDeferredRenderer::RenderGBuffer()
	{
		...
		for (auto& geometry : m_glGeometries)
		{
			...
			for (auto& glMesh : geometry->glModel->meshes)
			{
				...
				glUniformMatrix3fv(gBufferShader->GetUniform("normal_matrix"), 1, GL_FALSE, glm::value_ptr(glm::transpose(glm::inverse(glm::mat3(m)))));
				...
				glUniform1i(gBufferShader->GetUniform("metallic_roughness_texcoord_index"), materialData->metallicRoughnessTexCoordIndex);
	...
	void GLDeferredRenderer::RenderDirectionalLights()
	{
		for (auto& light : m_glDirectionalLights)
		{
			...
			glUniform3fv(m_deferredDirectionalLightShader->GetUniform("view_pos"), 1, glm::value_ptr(m_camera->GetWorldTranslation()));
			glUniform3fv(m_deferredDirectionalLightShader->GetUniform("light_pos"), 1, glm::value_ptr(light->node->GetWorldTranslation()));
			glUniform3fv(m_deferredDirectionalLightShader->GetUniform("light_color"), 1, glm::value_ptr(light->node->GetColor()));
			glUniform1f(m_deferredDirectionalLightShader->GetUniform("light_intensity"), light->node->GetIntensity());

			glUniformMatrix4fv(m_deferredDirectionalLightShader->GetUniform("light_space_matrix"), 1, GL_FALSE, glm::value_ptr(light->lightSpaceMatrix));
	...
	void GLDeferredRenderer::RenderSpotLights()
	{
		for (auto& light : m_glSpotLights)
		{
			...
			glUniform3fv(m_deferredSpotLightShader->GetUniform("view_pos"), 1, glm::value_ptr(m_camera->GetWorldTranslation()));
			glUniform3fv(m_deferredSpotLightShader->GetUniform("light_pos"), 1, glm::value_ptr(light->node->GetWorldTranslation()));
			...
			glUniformMatrix4fv(m_deferredSpotLightShader->GetUniform("light_space_matrix"), 1, GL_FALSE, glm::value_ptr(light->lightSpaceMatrix));
