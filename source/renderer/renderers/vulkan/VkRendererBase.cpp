...
void VkRendererBase::CreateSwapChain()
{
	...
	auto r = vkCreateSwapchainKHR(m_device, &createInfo, nullptr, &m_swapChain);
	vkCall(r);
...
void VkRendererBase::CreateDescriptorPool()
{
	...
	vkCall(vkCreateDescriptorPool(m_device, &pool_info, nullptr, &m_descriptorPool));
}
...
void VkRendererBase::RecordRenderCommandBuffer(VkCommandBuffer& buffer, uint32 imageIndex)
{
	VkCommandBufferBeginInfo beginInfo = { VK_STRUCTURE_TYPE_COMMAND_BUFFER_BEGIN_INFO };
	beginInfo.flags = 0;
	beginInfo.pInheritanceInfo = nullptr;
	...
	renderPassInfo.renderArea.offset = { 0, 0 };
	renderPassInfo.renderArea.extent = m_swapChainExtent;
	VkClearValue clearColor = { 0.0f, 0.0f, 0.1f, 0.0f };
	renderPassInfo.clearValueCount = 1;
	renderPassInfo.pClearValues = &clearColor;
	...
	// draw call (triangle)
	vkCmdDraw(buffer, static_cast<uint32>(m_vertices.size()), 1, 0, 0);
...
initialization (e.g. if swap chain is recreated)
...
void VkRendererBase::Render()
{
	...
	RecordRenderCommandBuffer(m_commandBuffers[imageIndex], imageIndex);
