...
BasePodHandle ImageImporter::Import(const fs::path& path)
{
	...
	auto& [handle, didCreate] = AssetImporterManager::CreateEntry<ImagePod>(finalPath, path.filename().string());

	if (didCreate) {
		auto pod = handle.Z_GetMutable();
		...
		CLOG_ABORT(hasNotResult, "ImagePod loading failed, filepath: {}, data_empty: {} width: {} height: {}",
			finalPath, static_cast<bool>(data), pod->width, pod->height);
