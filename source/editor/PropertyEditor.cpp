...
struct ReflectionToImguiVisitor {
	...
	//		handle = AssetImporterManager::ResolveOrImport<T>(uri::SystemToUri(propedit->m_openAsset.filepath));
	//		result = true;
	//	}
	//	else if (propedit->m_saveAsset.HasFileFor(currentObject, *currentProperty, id)) {
	//		auto& path = propedit->m_openAsset.filepath;
	//		SaveAs(handle, path);
	//		handle = AssetImporterManager::ResolveOrImport<T>(uri::SystemToUri(path));
	//		AssetImporterManager::Reload(handle);
	//		result = true;
	//	}
	//	return result;
