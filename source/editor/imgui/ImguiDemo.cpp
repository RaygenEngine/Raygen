...
// - Call and read ImGui::ShowDemoWindow() in imgui_demo.cpp for demo code. All applications in examples/ are doing
// that. Read imgui.cpp for more details, documentation and comments. Get latest version at
...
// if you don't call ImGui::ShowDemoWindow(). If you want to link core Dear ImGui in your shipped builds but want a
// thorough guarantee that the demo will not be linked, you can setup your imconfig.h with #define
// IMGUI_DISABLE_DEMO_WINDOWS and those functions will be empty. In other situation, whenever you have Dear ImGui
// available you probably want this to be available for reference. Thank you, -Your beloved friend, imgui_demo.cpp
...
// API (imgui.h).
// - We never use maths operators on ImVec2/ImVec4. For other of our sources files, they are provided by
// imgui_internal.h w/ IMGUI_DEFINE_MATH_OPERATORS.
...
void ImGui::ShowDemoWindow(bool* p_open)
{
	...
	// (default)
	ImGui::PushItemWidth(
		ImGui::GetFontSize() * -12); // Use fixed width for labels (by passing a negative value), the rest goes to
	...
	if (ImGui::CollapsingHeader("Configuration")) {
		...
		if (ImGui::TreeNode("Configuration##2")) {
			...
			ImGui::CheckboxFlags(
				"io.ConfigFlags: DockingEnable", (unsigned int*)&io.ConfigFlags, ImGuiConfigFlags_DockingEnable);
			ImGui::SameLine();
			...
			ImGui::CheckboxFlags(
				"io.ConfigFlags: ViewportsEnable", (unsigned int*)&io.ConfigFlags, ImGuiConfigFlags_ViewportsEnable);
			ImGui::SameLine();
...
static void ShowDemoWindowWidgets()
{
	...
	if (ImGui::TreeNode("Text")) {
		...
		if (ImGui::TreeNode("Word Wrapping")) {
			...
			ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(pos.x + wrap_width, pos.y),
				ImVec2(pos.x + wrap_width + 10, pos.y + ImGui::GetTextLineHeight()), IM_COL32(255, 0, 255, 255));
		...
		if (ImGui::TreeNode("UTF-8 Text")) {
			...
			ImGui::TextWrapped(
				...
				"io.Font->AddFontFromFileTTF() manually to load extra character ranges. Read docs/FONTS.txt for "
				"details.");
			ImGui::Text(
	...
	if (ImGui::TreeNode("Selectables")) {
		...
		if (ImGui::TreeNode("Grid")) {
			static bool selected[4 * 4] = { true, false, false, false, false, true, false, false, false, false, true,
	...
	if (ImGui::TreeNode("Text Input")) {
		...
		if (ImGui::TreeNode("Resize Callback")) {
			...
			// using your prefered type. See misc/cpp/imgui_stdlib.h for an implementation of this using std::string.
			HelpMarker(
	...
	if (ImGui::TreeNode("Color/Picker Widgets")) {
		...
		if (!alpha)
			flags |= ImGuiColorEditFlags_NoAlpha; // This is by default if you call ColorPicker3() instead of
												  // ColorPicker4()
		if (alpha_bar)
...
#	endif
		...
		ImGui::DragScalar(
			"drag double ^2", ImGuiDataType_Double, &f64_v, 0.0005f, &f64_zero, &f64_one, "0 < %.10f < 1", 2.0f);
		...
		ImGui::SliderScalar(
			"slider double low", ImGuiDataType_Double, &f64_v, &f64_zero, &f64_one, "%.10f grams", 1.0f);
	...
	if (ImGui::TreeNode("Drag and Drop")) {
		...
		if (ImGui::TreeNode("Drag to reorder items (simple)")) {
			// Simple reordering
			HelpMarker(
				"We don't use the drag and drop api at all here! Instead we query when the item is held but not "
	...
	if (ImGui::TreeNode("Querying Status (Active/Focused/Hovered etc.)")) {
		...
		ImGui::Combo("Item Type", &item_type,
			...
			"repeat)"
			"\0Checkbox\0SliderFloat\0InputText\0InputFloat\0InputFloat3\0ColorEdit4\0MenuItem\0TreeNode\0TreeNode (w/ "
		...
		ImGui::BulletText(
			...
			ImGui::GetItemRectMin().y, ImGui::GetItemRectMax().x, ImGui::GetItemRectMax().y, ImGui::GetItemRectSize().x,
...
static void ShowDemoWindowLayout()
{
	...
	if (ImGui::TreeNode("Text Baseline Alignment")) {
...

		{
			...
			bool node_open = ImGui::TreeNode("Node##2"); // Common mistake to avoid: if we want to SameLine after
	...
	if (ImGui::TreeNode("Clipping")) {
		...
		ImGui::GetWindowDrawList()->AddText(ImGui::GetFont(), ImGui::GetFontSize() * 2.0f,
			ImVec2(pos.x + offset.x, pos.y + offset.y), IM_COL32(255, 255, 255, 255), "Line 1 hello\nLine 2 clip me!",
			NULL, 0.0f, &clip_rect);
		ImGui::TreePop();
	}
}
...
static void ShowDemoWindowMisc()
{
	...
	if (ImGui::CollapsingHeader("Inputs, Navigation & Focus")) {
		...
		if (ImGui::TreeNode("Dragging")) {
			...
			if (ImGui::IsItemActive())
				ImGui::GetForegroundDrawList()->AddLine(io.MouseClickedPos[0], io.MousePos,
					ImGui::GetColorU32(ImGuiCol_Button), 4.0f); // Draw a line between the button and the mouse cursor
		...
		if (ImGui::TreeNode("Mouse cursors")) {
			...
			ImGui::Text(
				"Current mouse cursor = %d: %s", ImGui::GetMouseCursor(), mouse_cursors_names[ImGui::GetMouseCursor()]);
...
void ImGui::ShowAboutWindow(bool* p_open)
{
	...
	if (show_config_info) {
		...
		ImGui::BeginChildFrame(
			ImGui::GetID("cfginfos"), ImVec2(0, ImGui::GetTextLineHeightWithSpacing() * 18), ImGuiWindowFlags_NoMove);
...
void ImGui::ShowStyleEditor(ImGuiStyle* ref)
{
	...
	ImGui::SameLine();
	HelpMarker(
		"Save/Revert in local non-persistent storage. Default Colors definition are not affected. Use \"Export\" below "
		...
		if (ImGui::BeginTabItem("Colors")) {
			...
			for (int i = 0; i < ImGuiCol_COUNT; i++) {
				...
				if (memcmp(&style.Colors[i], &ref->Colors[i], sizeof(ImVec4)) != 0) {
					...
					// so instead of "Save"/"Revert" you'd use icons. Read the FAQ and docs/FONTS.txt about using icon
				...
				if (font_details_opened) {
					...
					if (ImGui::TreeNode("Glyphs", "Glyphs (%d)", font->Glyphs.Size)) {
						...
						for (unsigned int base = 0; base <= IM_UNICODE_CODEPOINT_MAX; base += 256) {
							...
							if (count > 0
								&& ImGui::TreeNode((void*)(intptr_t)base, "U+%04X..U+%04X (%d %s)", base, base + 255,
...
struct ExampleAppConsole {
	...
	ExampleAppConsole()
	{
		...
		Commands.push_back("CLASSIFY"); // "classify" is only here to provide an example of "C"+[tab] completing to "CL"
...
static void ShowExampleAppLongText(bool* p_open)
{
	...
	ImGui::Combo("Test type", &test_type,
		"Single call to TextUnformatted()\0Multiple calls to Text(), clipped\0Multiple calls to Text(), not clipped "
		"(slow)\0");
	...
	if (ImGui::BeginTabBar("##TabBar")) {
		...
		if (ImGui::BeginTabItem("Primitives")) {
			...
			for (int n = 0; n < 2; n++) {
				...
				draw_list->AddTriangle(
					ImVec2(x + sz * 0.2f, y), ImVec2(x, y + sz - 0.5f), ImVec2(x + sz * 0.4f, y + sz - 0.5f), col, th);
			...
			x += sz + spacing; // Triangle
			draw_list->AddTriangleFilled(
				ImVec2(x + sz * 0.2f, y), ImVec2(x, y + sz - 0.5f), ImVec2(x + sz * 0.4f, y + sz - 0.5f), col);
			x += sz * 0.4f + spacing; // Thin triangle
		...
		if (ImGui::BeginTabItem("Canvas")) {
			...
			draw_list->AddRectFilledMultiColor(canvas_pos,
				ImVec2(canvas_pos.x + canvas_size.x, canvas_pos.y + canvas_size.y), IM_COL32(50, 50, 50, 255),
			...
			for (int i = 0; i < points.Size - 1; i += 2)
				draw_list->AddLine(ImVec2(canvas_pos.x + points[i].x, canvas_pos.y + points[i].y),
					ImVec2(canvas_pos.x + points[i + 1].x, canvas_pos.y + points[i + 1].y), IM_COL32(255, 255, 0, 255),
					2.0f);
		...
		if (ImGui::BeginTabItem("BG/FG draw lists")) {
			...
			if (draw_bg)
				ImGui::GetBackgroundDrawList()->AddCircle(
					window_center, window_size.x * 0.6f, IM_COL32(255, 0, 0, 200), 48, 10 + 4);
			if (draw_fg)
				ImGui::GetForegroundDrawList()->AddCircle(
					window_center, window_size.y * 0.6f, IM_COL32(0, 255, 0, 200), 48, 10);
			ImGui::EndTabItem();
		}
